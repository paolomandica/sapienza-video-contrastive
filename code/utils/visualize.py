...
def draw_matches(x1, x2, i1, i2):
    # x1, x2 = f1, f2/
    def detach(x): return x.detach().cpu().numpy().transpose(1, 2, 0) * 255
    ...
    for check in [True]:
        ...
        grid = torch.stack([torch.arange(0, h)[None].repeat(
            h, 1), torch.arange(0, h)[:, None].repeat(1, h)])
...
class Visualize(object):
    ...
    def wandb_init(self, model):
        if not self._init:
            self._init = True
            wandb.init(project="teacher-student-superpixels", 
...
def nn_patches(vis, P, A_k, prefix='', N=10, K=20):
    ...
    for n, i in enumerate(np.random.permutation(P.shape[0])[:N]):
        ...
        for k in range(I.shape[0]):
            vis.images(P[I[k, i, :K]], nrow=min(I.shape[-1], 20),
                       win='%s_patch_values_%s_%s' % (prefix, n, k))
            vis.bar(A[k, i][::-1][:K], opts=dict(height=150, width=500),
                    win='%s_patch_affinity_%s_%s' % (prefix, n, k))
...
def frame_pair(x, ff, mm, t1, t2, A, AA, xent_loss, viz):
    def normalize(xx): return (xx-xx.min()) / (xx-xx.min()).max()
    ...
    if len(x.shape) < 6:   # Single image input, no patches
        ...
        pca1 = torchvision.utils.make_grid(torch.Tensor(
            pca_ff[:N]), nrow=int(N**0.5), padding=1, pad_value=1)
        pca2 = torchvision.utils.make_grid(torch.Tensor(
            pca_ff[N:]), nrow=int(N**0.5), padding=1, pad_value=1)
        img1 = torchvision.utils.make_grid(
            normalize(x1)*255, nrow=int(N**0.5), padding=1, pad_value=1)
        img2 = torchvision.utils.make_grid(
            normalize(x2)*255, nrow=int(N**0.5), padding=1, pad_value=1)
        viz.images(torch.stack([pca1, pca2]), nrow=4, win='pca_viz_combined1')
        viz.images(torch.stack([img1.cpu(), img2.cpu()]), opts=dict(
            title=f"{t1} {t2}"), nrow=4, win='pca_viz_combined2')
