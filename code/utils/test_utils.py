...
def process_pose(pred, lbl_set, topk=3):
    ...
    current_coord = torch.stack(
        [(xx * vals).sum(0), (yy * vals).sum(0)], dim=0)
...
def dump_predictions(pred, lbl_set, img, prefix):
    ...
    pred_soft = cv2.resize(
        pred_soft, (img.shape[1], img.shape[0]), interpolation=cv2.INTER_NEAREST)
...
def context_index_bank(n_context, long_mem, N):
    ...
    for t in long_mem:
        ...
        ll.append(idx)
    # "short" context
    ss = [(torch.arange(n_context)[None].repeat(
        N, 1) + torch.arange(N)[:, None])[:, :]]
...
def mem_efficient_batched_affinity(query, keys, mask, temperature, topk, long_mem, device):
    ...
    for b in range(0, keys.shape[2], bsize):
        _k, _q = keys[:, :, b:b +
                      bsize].to(device), query[:, :, b:b+bsize].to(device)
        w_s, i_s = [], []
