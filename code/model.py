...
class CRW(nn.Module):
    ...
    def zeroout_diag(self, A, zero=0):
        mask = (torch.eye(A.shape[-1]).unsqueeze(0).repeat(A.shape[0],
                1, 1).bool() < 1).float().cuda()
        return A * mask
    ...
    def forward(self, x, just_feats=False,):
        ...
        # shuffling the patches for each B and T
        perms = [torch.randperm(N) for _ in range(T)]
        for t, idx in enumerate(perms):
            q[:, :, t, :] = q[:, :, t, idx]
