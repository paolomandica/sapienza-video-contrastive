...
class CRW_vis(nn.Module):
    def __init__(self, args, vis=None):
        super(CRW_vis, self).__init__()
        self.args = args
    ...
    def vis_adj(self, video, sp_mask, As):

        T, C, H, W = video.shape

        fig, ax = plt.subplots(3, T, figsize=(20, 20)) #, sharex=True, sharey=True
        ...
        for t in range(T):
            ...
            for sp in np.unique(seg):
                ...
                for c in contours:
                    ...
                    ax[1, t].text(x, y, str(sp), fontsize=10)

            # pdb.set_trace()
            # display the image
            ax[0, t].imshow(mark_boundaries(np.transpose(img, (1,2,0)), seg))

            ax[1, t].imshow(seg)
            ax[1, t].scatter(X,Y, color='red')

            if t<T-1:
                ax[2, t].set_yticks(np.arange(0, As[t].shape[0], 2.0))
                ax[2, t].set_xticks(np.arange(0, As[t].shape[1], 2.0))
                im = ax[2, t].imshow(As[t])

        fig.delaxes(ax[2, T-1])

        divider = make_axes_locatable(ax[2, T-1])
        cax = divider.append_axes("left", size="10%", pad=0.05)

        plt.colorbar(im, cax=cax)

        fig.savefig('../adj_mat/adj_mat_visualization.png')
    ...
    def forward(self, x, sp_mask, max_sp_num, just_feats=False):
        ...
        self.vis_adj(x[0].cpu().detach().numpy(), sp_mask[0].cpu().detach().numpy(), torch.cat(A12s).cpu().detach().numpy())
